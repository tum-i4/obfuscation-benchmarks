...
{ 
  ...
  if ((state[0UL] >> 3U) & 1U) {
    ...
    *((char *)(& state[0UL]) + 2) = copy11;
    copy11 = *((char *)(& state[0UL]) + 2);
    *((char *)(& state[0UL]) + 2) = *((char *)(& state[0UL]) + 0);
    *((char *)(& state[0UL]) + 0) = copy11;
    state[0UL] *= state[0UL];
  } else {
    state[0UL] += state[0UL];
    state[0UL] |= (((state[0UL] >> ((state[0UL] & 15U) | 1UL)) | (state[0UL] << (32 - ((state[0UL] & 15U) | 1UL)))) & 31U) << 4UL;
  }
  output[0UL] = state[0UL] * 1367963592U;
}
}
void megaInit(void) 
{ 
