...
{ 
  ...
  if (output[0] == 18446744073469899601UL) {
    printf("You win!\n");
  } else {
...
{ 
  ...
  while (local1 < 0UL) {
    ...
    if (state[0UL] == local1) {
      ...
      *((char *)(& state[0UL]) + 4) = *((char *)(& state[0UL]) + 1);
      *((char *)(& state[0UL]) + 1) = copy14;
    } else {
      state[0UL] |= (state[0UL] * state[0UL] & 63UL) << 4UL;
      state[local1] = (state[0UL] << (((state[0UL] >> 4UL) & 15UL) | 1UL)) | (state[0UL] >> (64 - (((state[0UL] >> 4UL) & 15UL) | 1UL)));
    }
    local1 ++;
  }
  output[0UL] = (state[0UL] << 1UL) | (state[0UL] >> 63UL);
}
}
