...
{ 
  ...
  while (local1 < 0U) {
    ...
    if (state[0UL] < local1) {
      ...
      state[0UL] |= (((state[local1] << (((state[0UL] >> 3U) & 15U) | 1UL)) | (state[local1] >> (32 - (((state[0UL] >> 3U) & 15U) | 1UL)))) & 15U) << 3UL;
    } else {
      state[0UL] |= (state[local1] * state[0UL] & 31U) << 4UL;
  ...
  output[0UL] = state[0UL] * 424518743UL;
}
}
int main(int argc , char *argv[] ) 
{ 
