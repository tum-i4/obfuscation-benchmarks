...
{ 
  ...
  if (state[0UL] & 1UL) {
    ...
    if ((state[0UL] >> 1UL) & 1UL) {
      copy14 = *((unsigned int *)(& state[0UL]) + 1);
      *((unsigned int *)(& state[0UL]) + 1) = *((unsigned int *)(& state[0UL]) + 0);
      *((unsigned int *)(& state[0UL]) + 0) = copy14;
      copy14 = *((unsigned int *)(& state[0UL]) + 0);
      *((unsigned int *)(& state[0UL]) + 0) = *((unsigned int *)(& state[0UL]) + 1);
      *((unsigned int *)(& state[0UL]) + 1) = copy14;
    } else {
      state[0UL] *= state[0UL];
      copy16 = *((unsigned short *)(& state[0UL]) + 3);
      *((unsigned short *)(& state[0UL]) + 3) = *((unsigned short *)(& state[0UL]) + 0);
      *((unsigned short *)(& state[0UL]) + 0) = copy16;
    }
  } else
  if (state[0UL] & 1UL) {
    state[0UL] *= state[0UL];
    ...
    *((unsigned short *)(& state[0UL]) + 3) = copy18;
    copy18 = *((unsigned short *)(& state[0UL]) + 1);
    *((unsigned short *)(& state[0UL]) + 1) = *((unsigned short *)(& state[0UL]) + 0);
    *((unsigned short *)(& state[0UL]) + 0) = copy18;
  }
  output[0UL] = state[0UL] + 1506028036UL;
}
}
int main(int argc , char *argv[] ) 
{ 
