...
{ 
  ...
  while (local1 < input[1UL]) {
    ...
    if (state[0UL] <= local1) {
      state[0UL] |= ((state[local1] + state[0UL]) & (unsigned char)31) << 3UL;
    } else {
      state[local1] <<= ((state[0UL] >> (unsigned char)1) & (unsigned char)7) | 1UL;
    }
    local1 ++;
  }
  output[0UL] = (state[0UL] + 381482377UL) + (unsigned char)85;
}
}
void megaInit(void) 
{ 
