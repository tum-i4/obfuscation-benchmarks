...
{ 
  ...
  while (local1 < 1UL) {
    ...
    if (state[0UL] > local1) {
      state[local1] |= (state[local1] & 7UL) << 2UL;
      state[0UL] |= (state[0UL] * state[0UL] & 15UL) << 2UL;
    } else {
      copy13 = *((unsigned int *)(& state[0UL]) + 1);
      *((unsigned int *)(& state[0UL]) + 1) = *((unsigned int *)(& state[0UL]) + 0);
      *((unsigned int *)(& state[0UL]) + 0) = copy13;
      state[local1] |= (state[0UL] * state[local1] & 63UL) << 4UL;
    }
    local1 ++;
  }
  output[0UL] = (state[0UL] ^ 954804857UL) ^ 0x80000000009d9b96UL;
}
}
void megaInit(void) 
{ 
...
{ 
  ...
  if (output[0] == 1849001269864910352UL) {
    printf("You win!\n");
  } else {
