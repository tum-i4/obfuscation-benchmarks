...
{ 
  ...
  while (local1 < 0UL) {
    if (state[0UL] > local1) {
      state[local1] = state[0UL] << (((state[local1] >> 1UL) & 7UL) | 1UL);
    ...
    if (state[0UL] <= local1) {
      ...
      *((unsigned short *)(& state[local1]) + 2) = *((unsigned short *)(& state[local1]) + 1);
      *((unsigned short *)(& state[local1]) + 1) = copy14;
      copy15 = *((char *)(& state[local1]) + 6);
      *((char *)(& state[local1]) + 6) = *((char *)(& state[local1]) + 2);
      *((char *)(& state[local1]) + 2) = copy15;
    } else {
      copy16 = *((unsigned short *)(& state[0UL]) + 0);
      *((unsigned short *)(& state[0UL]) + 0) = *((unsigned short *)(& state[0UL]) + 2);
      *((unsigned short *)(& state[0UL]) + 2) = copy16;
      copy17 = *((char *)(& state[0UL]) + 7);
      *((char *)(& state[0UL]) + 7) = *((char *)(& state[0UL]) + 6);
      *((char *)(& state[0UL]) + 6) = copy17;
    }
    local1 ++;
  }
  output[0UL] = state[0UL] & 290058054UL;
}
}
