    ...
    if (state[0UL] > local1) {
      if (state[0UL] > local1) {
        ...
        *((char *)(& state[local1]) + 1) = copy11;
        ...
        state[local1] >>= ((state[0UL] >> (unsigned short)3) & (unsigned short)7) | 1UL;
        state[local1] >>= ((state[local1] >> (unsigned short)3) & (unsigned short)7) | 1UL;
      }
    } else
    if (! (state[0UL] == local1)) {
      state[0UL] >>= ((state[local1] >> (unsigned short)4) & (unsigned short)7) | 1UL;
      copy15 = *((char *)(& state[local1]) + 0);
      *((char *)(& state[local1]) + 0) = *((char *)(& state[local1]) + 1);
      *((char *)(& state[local1]) + 1) = copy15;
    }
    local1 ++;
  }
  output[0UL] = state[0UL] | 547249631UL;
}
}
