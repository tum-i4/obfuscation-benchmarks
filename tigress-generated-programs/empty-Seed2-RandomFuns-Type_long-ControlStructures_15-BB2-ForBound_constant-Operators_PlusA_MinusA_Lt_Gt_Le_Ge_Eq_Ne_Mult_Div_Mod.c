  ...
  while (local1 < 1UL) {
    ...
    while (local2 < 1UL) {
      if (state[0UL] > local2 + local1) {
        copy11 = *((unsigned short *)(& state[local2]) + 2);
        *((unsigned short *)(& state[local2]) + 2) = *((unsigned short *)(& state[local2]) + 3);
        *((unsigned short *)(& state[local2]) + 3) = copy11;
        copy12 = *((unsigned short *)(& state[0UL]) + 1);
        *((unsigned short *)(& state[0UL]) + 1) = *((unsigned short *)(& state[0UL]) + 2);
        *((unsigned short *)(& state[0UL]) + 2) = copy12;
      } else {
        copy13 = *((unsigned int *)(& state[local1]) + 1);
        *((unsigned int *)(& state[local1]) + 1) = *((unsigned int *)(& state[local1]) + 0);
        *((unsigned int *)(& state[local1]) + 0) = copy13;
        state[local2] = state[local1] + state[local2];
      }
      local2 += 2UL;
    }
    local1 += 2UL;
  }
  output[0UL] = (state[0UL] * 767205732UL) * 712285992UL;
}
}
void megaInit(void) 
{ 
...
{ 
  ...
  if (output[0] == 8411115195679836352UL) {
    printf("You win!\n");
  } else {
