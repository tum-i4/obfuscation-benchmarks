...
{ 
  ...
  while (local1 < input[1UL]) {
    ...
    if (state[0UL] < local1) {
      state[0UL] = state[local1] << (((state[local1] >> (unsigned short)1) & (unsigned short)7) | 1UL);
      ...
      *((char *)(& state[local1]) + 1) = copy11;
      ...
      state[0UL] |= (state[local1] & (unsigned short)63) << 4UL;
      state[local1] |= (state[0UL] & (unsigned short)15) << 2UL;
    }
    local1 ++;
  }
  output[0UL] = ((state[0UL] >> (unsigned short)1) | (state[0UL] << (unsigned short)15)) & (unsigned short)36958;
}
}
void megaInit(void) 
{ 
