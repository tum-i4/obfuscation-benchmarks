  ...
  while (local1 < (unsigned char)0) {
    ...
    while (local2 < (unsigned char)0) {
      if (state[0UL] == local2 * local1) {
        state[0UL] |= ((state[local1] | state[0UL]) & (unsigned char)15) << 3UL;
        state[local2] |= (state[0UL] & (unsigned char)7) << 2UL;
      } else {
        state[0UL] |= ((state[local1] + state[0UL]) & (unsigned char)31) << 3UL;
        state[local1] = state[local2] << (((state[0UL] >> (unsigned char)1) & (unsigned char)7) | 1UL);
      }
      local2 += 2UL;
    }
    local1 ++;
  }
  output[0UL] = state[0UL] * 501610228UL;
}
}
